...
of a linked list containing all of the elements in each list in sorted order.
...
One way to merge $k$ contiguous vectors together is to create a final output vector
...
The following code applies to linked lists, given you have a function to merge two
...
value and push it to our `finalList` vector. We then need increment the index we're
looking at in the vector we just took the min value from. This implies that we need
...
to `INT_MAX`, and a `minValueIndex` variable and iterate over each of the $k$ vector
...
elements of each vector, we then pick the smallest one. This would drop the time complexity down to
...
into our final structure. In fact this idea of a pairwise merging is exactly how `mergeSort` gets
...
with the previous naive method utilizing `mergeLists()`, the difference lies within the sizes of
...
is sum up all of the lists we're merging: $O(kn + k/2 \cdot 2n + k/4 \cdot 4n + ... + 2(nk/2) + 1nk)$.
The total amount of work each in each term is $nk$ because there are always $nk$ elements at play however
...
total number of elements we'd be sorting is $nk$ meaning the time complexity after sorting is $O(nk\log(nk))$.
...
we have to merge. This is why the variale inside the $\log(x)$ differs in the complexity between the two versions.

# Analysis

The following plot shows the results I received after running a randomized test in which I generate a multidimensional
vector of sorted vectors. Each iteration, both the multidimensional vector, and each sub-vector increases in length by
